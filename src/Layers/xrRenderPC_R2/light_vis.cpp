...
void	light::vis_prepare			()
{
	...
	float	viewport_near_new = 0.2;
	float	safe_area					= viewport_near_new;
	{
		float	a0	= deg2rad(Device.fFOV*Device.fASPECT/2.f);
		float	a1	= deg2rad(Device.fFOV/2.f);
		float	x0	= viewport_near_new /_cos	(a0);
		float	x1	= viewport_near_new /_cos	(a1);
		float	c	= _sqrt					(x0*x0 + x1*x1);
		safe_area	= _max(_max(viewport_near_new,_max(x0,x1)),c);
	}
