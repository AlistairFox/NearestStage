...
void update_rain_drops_factor(bool act_on_rain)
{
	...
	if (!fis_zero(rain_factor))
	{
		...
		const u32 change_step = 200; //Интервал в миллисекундах между ступенями изменения rain_drops_factor
		const u32 steps_count = 100; //Кол-во ступеней. Чем меньше интервал - тем больше ступеней должно быть.
		const float step_rain_factor_change = 1.f / float(steps_count);
		...
		//rain_drops_factor = std::max(rain_drops_factor, 0.f, rain_factor); //Уравниваем, чтобы не было превышения
	}
	else {
		steps_finished = 0;
