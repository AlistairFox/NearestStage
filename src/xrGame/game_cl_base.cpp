...
void	game_cl_GameState::net_import_GameTime		(NET_Packet& P)
{
	...
	float wfx_time;
	shared_str previosly, weather_name, WFX_name_import;
	...
	P.r_stringZ(WFX_name_import);
		...
		if (g_pGamePersistent)
		{
			...
			if (wfx_time > 0)
			{
				if (WFX_name_import.size() && !strstr(*GamePersistent().Environment().WFX_name, *WFX_name_import))
				{
					GamePersistent().Environment().StartWeatherFXFromTime(WFX_name_import, wfx_time);
					//Msg("Set Weather FX [%s] from time [%f]", WFX_name_import.c_str(), wfx_time);
				}
			}
			if (weather_name.size() && !strstr(*GamePersistent().Environment().CurrentWeatherName, *weather_name))
			{
				GamePersistent().Environment().Invalidate();
