...
void CSE_ALifeOnlineOfflineGroup::try_switch_online		()
{
	...
	for (auto it : m_members)
	{
		...
		if(CheckDistanceToActor(it.second->o_Position, alife().online_distance() )> alife().online_distance())
		{
			continue;
		}
		inherited1::try_switch_online	();
...
void CSE_ALifeOnlineOfflineGroup::try_switch_offline	()
{
	...
	for (auto it : m_members)
	{
		...
		if(CheckDistanceToActor(it.second->o_Position, alife().offline_distance()) <= alife().offline_distance())
			return;
	}
