...
void player_hud::SaveCfg(const int idx) const
{
	...
	pHudCfg.w_string(sect_name,
		"safemode_offset_pos_16x9",
		make_string("%f,%f,%f",
			hi->m_measures.m_safemodeoffset[0].x,
			hi->m_measures.m_safemodeoffset[0].y,
			hi->m_measures.m_safemodeoffset[0].z)
		.c_str());

	pHudCfg.w_string(sect_name,
		"safemode_offset_rot_16x9",
		make_string("%f,%f,%f",
			hi->m_measures.m_safemodeoffset[1].x,
			hi->m_measures.m_safemodeoffset[1].y,
			hi->m_measures.m_safemodeoffset[1].z)
		.c_str());
