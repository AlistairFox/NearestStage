...
float DistanceToActors(Fvector pos)
{
	...
	for (auto pl : Level().game->players)
	{
		...
		if (a)
		{
			float d = a->Position().distance_to_xz(pos);
			if (d < distance)
			{
				distance = d;
			}
	...
	{
		Fvector p = { 0,0,0 };
		distance = pos.distance_to(p);
	}

	return distance;
}

void CGameObject::update_check()
{
	if (smart_cast<CBolt*>(this) || smart_cast<CActor*>(this) || smart_cast<CSpectator*>(this))
	{
		need_update = true;
		dw_last_time = Device.dwTimeGlobal + 1000;
			return;
	}
	...
	if (g_dedicated_server)
		d = !H_Parent() ? DistanceToActors(Position()) : DistanceToActors(H_Parent()->Position());
	else
		d = !H_Parent() ? Device.vCameraPosition.distance_to_xz(Position()) : Device.vCameraPosition.distance_to_xz(H_Parent()->Position());
