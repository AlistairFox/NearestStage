...
public:
	...
	int						m_iInventoryFullnessCtrl; // ƒл€ контрол€ эвента. »наче эвент отправл€етс€ пачкой и дропает больше, чем нужно.
...
public:
	...
	s32						GetShotRndSeed			()	{ return m_ShotRndSeed;	};
		LPCSTR					ChatIcon				=						"no";
	void					SetChatIcon				(LPCSTR Icon);
	LPCSTR					GetChatIcon				()	{ return ChatIcon; };
