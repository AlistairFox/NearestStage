...
void CAI_Stalker::net_Export(NET_Packet& P)
{
	...
	else
	{
		...
		if (weapon && !weapon->strapped_mode())
		{
			...
			P.w_u16(inventory().ActiveItem()->object_id());
		}
		else
		{
...
void CAI_Stalker::net_Import(NET_Packet& P)
{
	...
	else
	{
		...
		SetfHealth(f_health);
		PIItem item = inventory().ItemFromSlot(u_active_weapon_slot);

		if (item && item->object_id() != u_active_weapon_id || !item)
		{
			CObject* obj = Level().Objects.net_Find(u_active_weapon_id);
			CInventoryItem* itemINV = smart_cast<CInventoryItem*>(obj);

			if (itemINV && itemINV->parent_id() == this->ID())
			{
				inventory().Slot(u_active_weapon_slot, itemINV, true, true);
			}
		}
