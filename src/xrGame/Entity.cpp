	...
	case GE_DIE:
		{
			...
			{
				if (this!=who)	/*if(bDebug) */ (cName().c_str(), who->cName().c_str() ); //Msg("%s killed by %s ...", cName().c_str(), who->cName().c_str());
				else			/*if(bDebug) */ (cName().c_str() ); //Msg("%s dies himself ...", cName().c_str());
			}
			Die				(who);
		}
		break;
