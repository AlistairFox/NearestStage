...
void CMovementManager::build_level_path	()
{
	...
	Msg									("build_level_path[%s][%f]",object().cName().c_str(),timer.GetElapsed_ms());
	/*
	if (old_time < Device.dwTimeGlobal)
	{
		old_time = Device.dwTimeGlobal + 1000;
			Msg("TimerPath: % d", global_timer);
		global_timer = 0;
}
	global_timer += timer.GetElapsed_ms();
	*/
}
