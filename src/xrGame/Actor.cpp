	...
	{	
		...
		for (; I <= E; ++I)
		{
			...
			else
			{
				...
				CCustomDetector* pDet = smart_cast<CCustomDetector*>(item_in_slot);
				if (pDet)
				{
					pDet->DestroyObject();
					continue;
...
void CActor::UpdateCL	()
{
	...
	if (pHelm && pHelm->IsDynamicHeldAvaliable)
		UseHelmet = true;
	else
		UseHelmet = false;
...
//				character_physics_support()->movement()->DestroyCharacter();c
...
void CActor::SetChatIcon(LPCSTR Icon)
{
	this->ChatIcon = Icon;
}
