...
CGamePersistent::CGamePersistent(void)
{
	...
	if (g_dedicated_server)
		DofValue.set(0.f, 0.f, 0.f);
	else
		DofValue.set(Console->GetFVector("r2_dof"));
	SetBaseDof				(DofValue);
}
...
float CGamePersistent::GetActorMaxThirst()
{
	if (!Actor())
		return 0;
	return (Actor()->conditions().GetThirstPower());
}

float CGamePersistent::GetActorThirst()
{
	if (!Actor())
		return 0;
